...
using Entity = Objects.BuiltElements.Entity;
...
namespace Objects.Converter.AutocadCivil
{
    ...
    public Alignment AlignmentToSpeckle(CivilDB.Alignment alignment)
    {
        ...
        // get the alignment entity curves
        List<Entity> entities = new List<Entity>();
        var stations = new List<double>();
        _alignment.entities = GetEntities(alignment);   
    ...
    public IEnumerable<AlignmentEntity> GetEntities(CivilDB.Alignment alignment)
    {
        foreach (var entity in alignment.Entities)
        {
            ...
            switch (entity.EntityType)
            {
                ...
                #endregion
            _entity.subEntities = GetSubEntities(entity,alignment);
    ...
    public IEnumerable<AlignmentSubEntity> GetSubEntities(CivilDB.AlignmentEntity entity, CivilDB.Alignment alignment)
    {
        for (int i = 0; i < entity.SubEntityCount; i++)
        {
            ...
            switch (entity[i].SubEntityType)
            {
                case CivilDB.AlignmentSubEntityType.Arc:
                    {
                        _subEntity = AlignmentArcToSpeckle(entity[i] as CivilDB.AlignmentSubEntityArc);
                        break;
                    }
                case CivilDB.AlignmentSubEntityType.Line:
                    {
                        _subEntity = AlignmentLineToSpeckle(entity[i] as CivilDB.AlignmentSubEntityLine);
                        break;
                    }
                case CivilDB.AlignmentSubEntityType.Spiral:
                    {
                        _subEntity = AlignmentSpiralToSpeckle(entity[i] as CivilDB.AlignmentSubEntitySpiral,alignment);
                        break;
                    }
    ...
    public CivilDB.Alignment AlignmentToNative(Alignment alignment)
    {
      ...
      foreach (var curve in alignment.entities)
        AddAlignmentSubEntity(curve, ref entities);

      return _alignment;
    }

#region helper methods
    private void AddAlignmentSubEntity(ICurve curve, ref CivilDB.AlignmentEntityCollection entities)
    {
      switch (curve)
      {
        case Line o:
        ...
        case Polycurve o:
          foreach (var segment in o.segments)
            AddAlignmentSubEntity(segment, ref entities);
          break;
